...
as $$
  select
    (select count(*) from public.daily_question_answers a where a.question_id = p_question_id) >= 2
    and exists (
...
begin
  ...
  select count(*)::integer into answer_count from public.daily_question_answers a where a.question_id = p_question_id;
...
grant select on public.daily_question_answers to authenticated;

drop policy if exists "Daily questions are visible to space members" on public.daily_questions;
create policy "Daily questions are visible to space members" on public.daily_questions
for select to authenticated using (space_id in (select private.user_space_ids()));

drop policy if exists "Users see their own answer or revealed answers" on public.daily_question_answers;
create policy "Users see their own answer or revealed answers" on public.daily_question_answers
for select to authenticated using (
  user_id = (select auth.uid()) or private.is_daily_question_revealed(question_id)
);
